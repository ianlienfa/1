...
string Judge::getData(int question)
{
    ...
    if(question == 0)
    {
        in.open("C:\\Users\\USER\\Desktop\\AOOPNCTU\\Lab\\Final Project\\Lab6\\0.txt"); //若相對位置，要將0.txt與1.exe擺在同個directory!!!
    ...
    if(question == 1)
    {
        in.open("C:\\Users\\USER\\Desktop\\AOOPNCTU\\Lab\\Final Project\\Lab6\\1.txt"); //txt檔每一個line最後不能有space!!!，否則string to int會出錯
        //if(!in){qDebug() << "dddd";}
        while(getline(in,input_data))
        {
